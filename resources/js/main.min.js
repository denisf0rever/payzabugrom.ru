(function () {
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const selectWrapper = document.querySelectorAll(
      ".calculation-form__select-wrapper"
    );
    const select = document.querySelectorAll(
      ".calculation-form__status-select"
    );
    const selectArrow = document.querySelectorAll(
      ".calculation-form__status-arrow"
    );
    const selectOptions = document.querySelectorAll(
      ".calculation-form__status-option"
    );
    const selectInput = document.querySelectorAll(
      ".calculation-form__status-current"
    );
    ...
    selectWrapper.forEach((el, key) => {
      const selectOptions = el.querySelectorAll(
        ".calculation-form__status-option"
      );
          ...
          selectOptions.forEach((innerEl, innerKey) => {
            if (key === innerKey) {
              ...
              innerEl.classList.remove(
                "calculation-form__status-option-active"
              );
            }
          });
        };
    ...
    const calcPrice = () => {
      const priceDisplay = document.querySelector(
        ".calculation-display__price"
      );
      const priceInput = document.querySelector(".calculation-form__value");
      const formPrice = document.querySelector(
        ".calculation-display__price-input"
      );
      const comissionDisplay = document.querySelector(
        ".calculation-display__tax-value"
      );
      const currency = document.querySelector("#currency");
      const options = document.querySelector("#options");

      let currencyModificator = 87.1;
      let comission = 10;
      ...
      const setPrice = () => {
        ...
        ${Math.round(
          priceInput.value * currencyModificator +
            (priceInput.value * currencyModificator * comission) / 100
        )} ₽
        `;
      };
